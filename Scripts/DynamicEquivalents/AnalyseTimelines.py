...
if __name__ == "__main__":
    ...
    '   - Timeline files are in a "timeLine/" subfolder of working_dir (as created in a SA)'
    ...
    for filename in filenames:
        ...
        output['OverSpeed trips'] = sum(1 for (_, __, event) in trip_timeline if 'OverSpeed' in event)
        output['UFLS actions'] = sum(1 for (_, __, event) in trip_timeline if 'activated' in event)
